...
import type { TimeSession, TimerState, DayStats } from '../types'
import {
  initDatabase,
  saveSession,
  updateSession,
...
export function useTimeTracker() {
  ...
  const loadActiveSession = async () => {
    try {
      ...
      error.value = `Failed to load active session: ${err instanceof Error ? err.message : 'Unknown error'}`
    }
  }
  ...
  const updateSessionData = async (session: TimeSession, updates: Partial<TimeSession>) => {
    try {
      ...
      error.value = `Failed to update session: ${err instanceof Error ? err.message : 'Unknown error'}`
    } finally {
      loading.value = false
    }
  }
  ...
  const deleteSessionData = async (session: TimeSession) => {
    try {
      ...
      error.value = `Failed to delete session: ${err instanceof Error ? err.message : 'Unknown error'}`
    } finally {
      loading.value = false
    }
  }
  ...
  const addManualSession = async () => {
    ...
    try {
      ...
      if (overlapping.length > 0) {
        throw new Error('Default time range overlaps with existing sessions. Please edit the times.')
      }
  ...
  const loadWeeklyStats = async () => {
    try {
      ...
      error.value = `Failed to load weekly stats: ${err instanceof Error ? err.message : 'Unknown error'}`
    }
  }
