...
export async function getActiveSession(): Promise<TimeSession | null> {
  ...
  const activeSession = await index.get(true)
  await tx.done
...
export async function getSessionsInDateRange(startDate: string, endDate: string): Promise<TimeSession[]> {
  const database = await initDatabase()
  const tx = database.transaction(SESSION_STORE, 'readonly')
  const store = tx.objectStore(SESSION_STORE)
  const index = store.index('date')
...
export async function calculateDayStats(date: string): Promise<{ totalDuration: number; sessionCount: number }> {
